...
`index.html` einfach **per Doppelklick** im Browser öffnen (`file://` reicht) — kein Python,
...
```jsonc
{
  ...
  "water": { "type": "flat", "tempC": 21, "wetsuit": "Shorty" },               // type: flat|wave|lagoon|choppy|mixed
...
### Pflichtfelder
`id, name, spot, region, country, verdict, verdictLabel, tagline, scores{kite,family,infra,access},
wind{min,max,type,reliability}, water{type,tempC,wetsuit}, season{1..12},
distanceUAE{time,transfers,note,hours}, distanceGermany{…}`

`windChart`, `subspots`, `family`, `tags`, `flagNote`, `level`, `flag` sind optional.
`windChart` setzen die Generatoren (siehe unten); `hours` (Flugstunden) trägst du in `distance*` selbst ein.
...
| Feld | Quelle |
|---|---|
| **`windChart`** (Monatsdiagramm) + **`reliability`** (WWK-Spots) | **WhenWhereKite** — echte Werte, aus dem Seitenquelltext (`sampleData1`) gezogen. **Kiter-Feedback**, kein Messinstrument, bildet aber lokale Effekte ab. **25 Spots.** |
